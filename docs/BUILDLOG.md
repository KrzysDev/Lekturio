...
But then there was another issue I have not tought about - halucination. The model sometimes wrote things in.. chinese or things like 
```txt
<br> <br> <br> <br>...and repeat that like milion times
```

Finally i found a library that preserves the formatting of the page, has ocr fallback and has very little syntax. On top of that, I also tried to look for other pdf's because what if there is something wrong with them? And I was right - the pdf's were the main issue. I still don't know what it is yet, but the same extracting tool gives much better results on diffrent pdf.
